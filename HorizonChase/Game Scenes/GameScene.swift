...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        rightArrow.position = CGPoint(x: visibleFrame.maxX - 60, y: visibleFrame.minY + 60)
        ...
        leftArrow.position = CGPoint(x: visibleFrame.minX + 60, y: visibleFrame.minY + 60)
    ...
    private func moveCarRight() {
        ...
        if newPositionX <= frame.maxX - shoulderWidth - racingCar.size.width / 2 {
    ...
    private func moveCarLeft() {
        let newPositionX = racingCar.position.x - laneWidth
        if newPositionX >= frame.minX + shoulderWidth + racingCar.size.width / 2 {
            let moveAction = SKAction.moveTo(x: newPositionX, duration: 0.2)
            racingCar.run(moveAction)
        }
    }
